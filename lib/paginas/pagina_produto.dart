...
class PaginaProduto extends StatefulWidget with PaginaEntidade {
  PaginaProduto({Key? key,required OperacaoCadastro operacaoCadastro,
    required Entidade entidade}) : super(key: key){
...
class _PaginaProdutoState extends State<PaginaProduto> with EstadoPaginaEntidade{
  ...
  ControleCadastroTipoProduto _controleTipoProduto = ControleCadastroTipoProduto();
  ...
  void _carregarTiposProdutos() {
    _itensTipoProduto = [];
    _controleTipoProduto.selecionarTodos().then(
            (listaTiposProdutos) {
          listaTiposProdutos.map((entidade) {
          ...
          }).forEach((itemTipoProduto) {
            _itensTipoProduto.add(itemTipoProduto);
          });
        });
  }
    ...
    return [TextField(
      ...
      DropdownButton(
        ...
        onChanged: (unidade){
          ...
          setState(() {
            (widget.entidade as Produto).unidade =
            unidade as String;
          });
        },
      ),
      DropdownButton(
        ...
        value: (widget.entidade as Produto).tipoProduto,
        onChanged: (tipoProduto){
          //É necessário ter o onChanged para funcionar.
          setState(() {
            (widget.entidade as Produto).tipoProduto =
